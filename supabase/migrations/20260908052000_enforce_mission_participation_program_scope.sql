...
as $$
  with caller_access as (
    select
      ...
      public.is_tenant_content_manager(p_tenant_id) as can_manage_tenant
  ),
  active_programs as (
    select
      p.id,
    ...
    where p.tenant_id = p_tenant_id
      and (select ca.can_manage_tenant from caller_access ca)
      and (
        (select ca.is_platform_admin or ca.is_owner from caller_access ca)
        or exists (
          select 1
