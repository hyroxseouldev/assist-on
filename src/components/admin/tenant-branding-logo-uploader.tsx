...
type TenantBrandingLogoUploaderProps = {
  tenantId: string;
  teamName: string;
  logoUrl: string;
...
export function TenantBrandingLogoUploader({ tenantId, teamName, logoUrl, onUploaded }: TenantBrandingLogoUploaderProps) {
  ...
  const handleUpload = async (file: File) => {
    ...
    const uploaded = await uploadImageToStorage(file, {
      ...
      userId: user.id,
      domainFolder: "tenant-branding-logo",
      maxDimension: 1024,
      quality: 0.9,
    });
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    startTransition(async () => {
      try {
        const nextUrl = await handleUpload(file);
        onUploaded(nextUrl);
        toast.success("브랜딩 로고가 업로드되었습니다.");
      } catch (error) {
        const message = error instanceof Error ? error.message : "브랜딩 로고 업로드에 실패했습니다.";
        toast.error(message);
      } finally {
        event.target.value = "";
      }
  ...
  return (
    <div className="space-y-3 rounded-md border bg-zinc-50 p-3 md:col-span-2">
      <p className="text-sm font-medium text-zinc-900">브랜딩 로고</p>
      <div className="flex items-center gap-4">
        <div className="relative h-16 w-16 overflow-hidden rounded-xl border border-zinc-200 bg-white">
          <Image src={currentLogoUrl} alt={`${teamName || "브랜딩"} 로고`} fill className="object-cover" />
        </div>
