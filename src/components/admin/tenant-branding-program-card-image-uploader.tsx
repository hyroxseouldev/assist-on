...
type TenantBrandingProgramCardImageUploaderProps = {
  tenantId: string;
  teamName: string;
  imageUrl: string;
...
export function TenantBrandingProgramCardImageUploader({
  tenantId,
  teamName,
  imageUrl,
  onUploaded,
}: TenantBrandingProgramCardImageUploaderProps) {
  ...
  const handleUpload = async (file: File) => {
    ...
    const uploaded = await uploadImageToStorage(file, {
      ...
      userId: user.id,
      domainFolder: "tenant-branding-program-card",
      maxDimension: 1600,
      quality: 0.9,
    });
  ...
  const handleCropConfirm = (croppedFile: File) => {
    startTransition(async () => {
      try {
        ...
        onUploaded(nextUrl);
        toast.success("프로그램 카드 이미지가 업로드되었습니다.");
        setCropSourceFile(null);
        setIsCropDialogOpen(false);
      } catch (error) {
        const message = error instanceof Error ? error.message : "프로그램 카드 이미지 업로드에 실패했습니다.";
        toast.error(message);
      }
    });
  };
  ...
  return (
    <div className="space-y-3 rounded-md border bg-zinc-50 p-3 md:col-span-2">
      <p className="text-sm font-medium text-zinc-900">프로그램 카드 이미지</p>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <div className="relative aspect-video w-full max-w-sm overflow-hidden rounded-md border border-zinc-200 bg-white">
          {imageUrl ? (
            <Image src={imageUrl} alt={`${teamName || "브랜딩"} 프로그램 카드 이미지`} fill className="object-cover" />
          ) : (
      ...
      <SquareImageCropDialog
        ...
        outputWidth={1600}
        outputHeight={900}
        title="프로그램 카드 이미지 16:9 크롭"
        description="드래그와 확대/축소로 카드 이미지 영역을 맞춰 주세요."
