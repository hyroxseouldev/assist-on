...
export async function generateMetadata({
  ...
  return buildTenantMetadata({
    tenantSlug,
    pageTitle: "개인정보처리방침",
    description: "개인정보처리방침 전문을 확인할 수 있는 페이지",
  });
}
...
export default async function TenantPrivacyPolicyPage({
  ...
  const { tenantSlug } = await params;
  const [document, siteData] = await Promise.all([
    getPublishedLegalDocumentByType(tenantSlug, "privacy_policy", "ko"),
    getTenantPublicSiteDataBySlug(tenantSlug),
  ]);
        ...
        <Card className="border-zinc-200/80 bg-white/95 shadow-sm">
          <CardHeader className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <p className="text-xs text-zinc-500">{brandLabel}</p>
            ...
            <CardTitle className="text-2xl tracking-tight text-zinc-900">{document.title || "개인정보처리방침"}</CardTitle>
          </CardHeader>
          <CardContent>
            <article
