      ...
      return (
        <main className="mx-auto w-full max-w-xl px-4 py-16">
          <Card>
            ...
            <CardContent>
              <Button asChild>
                <Link href={productId ? `/store/${tenantSlug}/${productId}/checkout${duration ? `?duration=${duration}` : ""}` : `/store/${tenantSlug}`}>돌아가기</Link>
              </Button>
            </CardContent>
          </Card>
        </main>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
              ...
              {order.duration_months ? <p className="mt-1">선택 옵션: {formatDurationPassLabel(order.duration_months)}</p> : null}
